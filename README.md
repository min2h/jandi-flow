...
이미 만든 `https://github.com/min2h/jandi-flow-test-repo` 로 이렇게 확인합니다.

1. GitHub PAT는 **classic `repo`** (private 읽기/쓰기). `public_repo`만 있으면 private 연결이 실패합니다.
2. [http://localhost:5173](http://localhost:5173) 에서 PAT 로그인
3. **기존 레포 연결**에 `https://github.com/min2h/jandi-flow-test-repo` 붙여넣고 연결 확인
4. 상태 타일이 `연결됨` / `private` 인지 확인
5. `지금 심기` = 연한 잔디(1커밋). `오늘 불태우기` = 진한 잔디(여러 커밋)
6. 해당 private 레포 Commits 와 프로필 잔디칸을 확인

PAT 값은 채팅에 붙이지 마세요.

연결은 저장 전과 매일 실행 직전에 다시 검증합니다. 레포가 사라지거나 토큰/키가 바뀌면 심지 않고 상태 타일에 표시합니다.

로그인 화면의 사과·달·토끼·유령은 **기능이 아니라 정원 분위기 장식**입니다. 연한 새싹/진한 잎 타일만 잔디 농도를 설명합니다.
